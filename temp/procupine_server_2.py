...
while True:
    ...
    if transcription >= 0:
        ...
        if AUDIO_RECOG:
            auth = False
            response = requests.post(AUDIO_RECOG_API, data= {'user': AUDIO_AUTH_USER}  ,  files={'audio': open(audio_clip_path, 'rb') })
            if response.status_code == 200:
                transcription = response.json()
                if transcription['Sim'] < 0.8:
                    auth = True
                    print('detected', transcription['Sim']  )
                else:
                    print('not detected', transcription['Sim']  )
            else:
